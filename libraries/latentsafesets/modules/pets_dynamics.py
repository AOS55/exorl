...
class ProbabilisticDynamicsModel(nn.Module):
    ...
    def forward(self, emb, acs):
        ...
        # Clip if the value of variance too small
        # TODO: look at just handling in try do loop if too small/large
        delta_normalized_logstd = torch.clip(delta_normalized_logstd, min=-100.0, max=80)
        ...
        try:
            ...
            print(f'low value after: {delta_normalized_logstd[delta_normalized_logstd <= -3.0]}')
            print(f'delta_std below bound: {delta_normalized_std[delta_normalized_std <= 0.0]}')
            print(f'delta_normalized_mean: {delta_normalized_mean}')
            print(f'delta_normalized_std: {delta_normalized_std}')
            print(f'emb: {emb}')
            print(f'acs: {acs}')
