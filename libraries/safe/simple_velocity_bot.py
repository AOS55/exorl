...
representation is (x, y, dx, dy). Action representation is (d2x, d2y).
"""
...
class SimpleVelocityBot(Env, utils.EzPickle):
    ...
    def step(self, a):
        ...
        cur_reward = self.step_reward(self.state[:2], a)
        self.state = next_state
        self._episode_steps += 1
        constr = self.obstacle(next_state[:2])
    ...
    def reset(self, random_start=False):
        ...
        if random_start:
            ...
            self.state[:2] = self.start_pos + np.random.randn(2)
            self.state[2:] = np.array([0.0, 0.0])
        self.done = False
        self._episode_steps = 0
        if self._from_pixels:
            obs = self._state_to_image(self.state)
    ...
    def _next_state(self, s, a, override=False):
        if self.obstacle(s[:2]):
            # TODO: ensure this does not cause the agent to become stuck on the obstacle 
            return s

        next_state = s
        next_state[2:] = s[2:] + a + self.noise_scale * np.random.randn(len(s[2:]))  # update velocity
        next_state[:2] = s[:2] + s[2:]  # update position
        next_state[:2] = np.clip(next_state[:2], (0, 0), (WINDOW_WIDTH, WINDOW_HEIGHT))  # ensure state remains in bounded box
        return next_state
    ...
    def _process_action(a):
        return np.clip(a, -MAX_ACCEL, MAX_ACCEL)
